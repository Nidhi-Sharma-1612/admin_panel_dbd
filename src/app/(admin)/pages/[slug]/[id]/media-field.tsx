  ...
  return (
    <div className="space-y-2">
      {value ? (
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                <Upload className="size-3.5" /> {pending ? "Uploading..." : "Upload from device"}
              </button>
            ...
            <button
              type="button"
              onClick={() => onChange("")}
              className="flex w-fit items-center gap-1.5 rounded-full border border-red-200 px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50"
            >
        ...
        <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed border-slate-300 p-4">
          <button
            type="button"
            onClick={() => inputRef.current?.click()}
            disabled={pending}
            className="flex h-12 w-full items-center justify-center gap-2 rounded-lg text-sm text-slate-500 hover:text-accent-dark disabled:opacity-60"
          >
            {pending ? (
              "Uploading..."
            ) : (
              <>
                <ImageOff className="size-4" /> Upload an image or video from device
              </>
            )}
          </button>
